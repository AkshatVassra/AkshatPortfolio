...
function TypewriterRole() {
  ...
  useEffect(() => {
    ...
    if (pause) {
      const t = setTimeout(() => { setDeleting(true); setPause(false); }, 1400);
  ...
  return (
    <span className="font-mono" style={{ background: 'linear-gradient(90deg,#a855f7,#06b6d4)', WebkitBackgroundClip: 'text', backgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
      {displayed}
      <motion.span
        animate={{ opacity: [1, 0] }}
        transition={{ duration: 0.5, repeat: Infinity }}
        style={{ display: 'inline-block', width: 2, height: '0.9em', background: '#06b6d4', marginLeft: 2, verticalAlign: 'middle' }}
      />
    </span>
  );
}
    ...
    <section className="relative min-h-screen flex items-center justify-center pt-20 px-6 overflow-hidden">
      ...
      >
        ...
        >
          ...
          >
            <span style={{ backgroundImage: 'linear-gradient(135deg,#a855f7 0%,#ec4899 50%,#06b6d4 100%)', WebkitBackgroundClip: 'text', backgroundClip: 'text', WebkitTextFillColor: 'transparent', display: 'block' }}>
              AKSHAT
            </span>
            <span style={{ backgroundImage: 'linear-gradient(135deg,#06b6d4 0%,#a855f7 50%,#ec4899 100%)', WebkitBackgroundClip: 'text', backgroundClip: 'text', WebkitTextFillColor: 'transparent', display: 'block' }}>
              VASSRA
            </span>
          </h1>
        </motion.div>
        ...
        >
          <motion.a
            ...
            style={{ background: 'linear-gradient(135deg,#a855f7,#ec4899)', willChange: 'transform' }}
          >
            View My Work <ArrowRight size={17} className="group-hover:translate-x-1 transition-transform duration-150" />
          </motion.a>
          <motion.a
            href="#contact"
            whileHover={{ scale: 1.04 }}
        ...
        >
          {STATS.map(s => (
            <motion.div
              ...
              style={{ background: 'rgba(168,85,247,0.06)', borderColor: 'rgba(168,85,247,0.22)', willChange: 'transform' }}
            >
              <div className="text-lg mb-0.5">{s.icon}</div>
              <p className="text-xs mb-0.5" style={{ color: 'rgba(255,255,255,0.5)' }}>{s.label}</p>
              <p className="text-base font-bold" style={{ color: '#a855f7' }}>{s.value}</p>
            </motion.div>
          ))}
        </motion.div>
        ...
        <motion.div
          ...
          style={{ willChange: 'transform' }}
        >
          <div className="w-6 h-10 border-2 rounded-full flex items-start justify-center p-2" style={{ borderColor: 'rgba(168,85,247,0.4)' }}>
            <motion.div
              animate={{ y: [0, 5, 0] }}
              transition={{ duration: 1.8, repeat: Infinity, ease: 'easeInOut' }}
